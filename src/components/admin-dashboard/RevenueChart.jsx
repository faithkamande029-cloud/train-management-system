...
const RevenueChart = memo(({ bookings }) => {
  ...
  return (
    <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-600 h-100">
      <h2 className="text-xl font-semibold mb-6 text-white">Revenue Overview</h2>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="#333" />
          <XAxis dataKey="name" tick={{ fill: "#94A3B8", fontSize: 12 }} />
          <Tooltip formatter={(value) => formatCurrency(value)} contentStyle={{ backgroundColor: "#D0312D", border: "1px solid #333" }} />
