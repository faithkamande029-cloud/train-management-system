...
const TicketCard = memo(({ booking, onSelect, onToggleFavorite, isFavorite }) => {
  ...
  return (
    <div className="bg-gray-900 rounded-2xl shadow-md overflow-hidden hover:shadow-red-900/30 hover:shadow-lg transition group cursor-pointer border border-gray-800" onClick={() => onSelect(booking)}>
      ...
      <div className="p-5">
        <h3 className="text-xl font-bold text-white mb-1">{booking.passengerName || "Unknown Passenger"}</h3>
        <div className="flex items-center gap-2 text-gray-400 text-sm mb-3">
          <FaMapMarkerAlt className="text-red-400" /><span>{booking.fromStation || "?"}</span>
          <span className="text-gray-600">→</span><span>{booking.toStation || "?"}</span>
        </div>
        <div className="flex justify-between text-gray-400 text-sm mb-4">
          <div className="flex items-center gap-1"><MdEventSeat className="text-red-400" /> Seat {booking.seatNumber || "?"}</div>
