...
test("accepts a newly registered account", () => {
  ...
  assert.equal(
    validateLogin("ADA@example.com", "secret1", "user", accounts).ok,
    true,
  );
});
...
test("validates sign-up requirements and duplicate emails", () => {
  assert.deepEqual(
    validateSignup("", "ada@example.com", "secret1", "secret1"),
    { ok: false, error: "Please enter your name." },
  );
  assert.deepEqual(
    validateSignup("Ada", "ada@example.com", "secret1", "mismatch"),
    { ok: false, error: "Passwords do not match." },
  );
  assert.deepEqual(
    validateSignup("Ada", "user@railway.com", "secret1", "secret1"),
    { ok: false, error: "An account with this email already exists." },
  );
  assert.deepEqual(
    validateSignup("Ada", "ada@example.com", "secret1", "secret1"),
    { ok: true, error: null },
  );
});

test("validates phone numbers during sign up", () => {
  assert.deepEqual(
    validateSignup("Ada", "ada@example.com", "secret1", "secret1", [], {
      firstName: "Ada",
      lastName: "Lovelace",
...
test("validates password reset against an existing account", () => {
  assert.deepEqual(
    validatePasswordReset("missing@example.com", "secret1", "secret1"),
    { ok: false, error: "No account was found for that email address." },
  );
  assert.deepEqual(
    validatePasswordReset("user@railway.com", "secret1", "secret1"),
    { ok: true, error: null },
  );
});
