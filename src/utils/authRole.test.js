...
test('accepts a registered account with a normalized email', () => {
...
test('validates sign-up fields and prevents duplicate emails', () => {
  assert.deepEqual(validateSignup('', 'ada@example.com', 'secret1', 'secret1'), { ok: false, error: 'Please enter your name.' });
  assert.deepEqual(validateSignup('Ada', 'ada@example.com', 'secret1', 'different'), { ok: false, error: 'Passwords do not match.' });
...
test('allows password reset only for an existing account', () => {
